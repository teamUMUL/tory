...
class STORepoImpl: STORepo {
    ...
    override suspend fun updateSto(stoInfo: StoResponse, updateStoRequest: UpdateStoRequest) : Response<StoResponse> {
        return stoRetrofit.updateSto(stoId = stoInfo.id, updateStoRequest = updateStoRequest)
    }
    ...
    override suspend fun updateImageList(
        ...
        return stoRetrofit.updateImageList(stoId = stoInfo.id, updateImageListRequest = updateImageListRequest)
    }

    override suspend fun getAllSTOs(studentId : Long): List<StoResponse> {
        return stoRetrofit.getStoList(studentId = studentId)
    }


    override suspend fun getSTOsByLTO(ltoId: Long): List<StoResponse> {
    ...
    override suspend fun addRound(selectedSTO: StoResponse, updateStoRoundRequest: UpdateStoRoundRequest) : Response<StoResponse> {
        return stoRetrofit.updateStoRound(stoId = selectedSTO.id, updateStoRoundRequest = updateStoRoundRequest)
    }
    override suspend fun addRoundHit(selectedSTO: StoResponse, updateStoRoundRequest: UpdateStoRoundRequest) : Response<StoResponse> {
        return stoRetrofit.updateStoHitRound(stoId = selectedSTO.id, updateStoRoundRequest = updateStoRoundRequest)
    }
